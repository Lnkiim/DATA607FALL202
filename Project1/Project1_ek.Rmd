...
library(dplyr)
library(stringr)
urlfile="/Users/ellenkim/CUNY/data_acquisition_management/Project1/Project1_ek/tournamentinfo.txt"
...
# separate first and second rows for each user
...
``` {r}
rounds_df1
# extract opponent's playerId for round 1
...
We only count W/L/D as games played. 

```{r}
rounds_df1$gamesPlayed <- NA
#rounds_df1 %>% add_column(dfs = NA)
...
masterdf_1$playerId <- as.numeric(as.character(masterdf_1$playerId))
